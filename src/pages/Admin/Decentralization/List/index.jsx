...
import { fetchGetAllDecentralization } from "../../../../redux/slice/decentralizationSlice";

const DecentralizationList = () => {
  document.title = "Danh sách quyền - Văn Quyết Mobile";
  const dispatch = useDispatch();
  const decentralizations = useSelector(
    (state) => state.decentralization.decentralizations,
  );
  ...
  useEffect(() => {
    dispatch(fetchGetAllDecentralization(filters))
      .unwrap()
      .then((res) => {
                ...
                <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white dark:divide-gray-700 dark:bg-gray-900">
                    {isLoading ? (
                      ...
                      decentralizations?.data?.length > 0 ? (
                      decentralizations?.data.map((decentralization) => (
                        <tr key={decentralization.id}>
                          <td className="whitespace-nowrap px-4 py-4 text-center text-sm font-medium text-gray-700 dark:text-gray-200">
                            <span>{decentralization.id}</span>
                          </td>
