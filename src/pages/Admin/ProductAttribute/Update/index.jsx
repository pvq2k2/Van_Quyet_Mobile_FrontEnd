...
import {
  fetchGetUpdateProductAttributeByID,
  fetchUpdateProductAttribute,
} from "../../../../redux/slice/productAttributeSlice";

const ProductAttributeUpdate = () => {
  document.title = "Cập nhật thuộc tính sản phẩm - Văn Quyết Mobile";
  const { productId, productAttributeId } = useParams();
  ...
  const handleSubmit = async (value) => {
    ...
    try {
      const res = await dispatch(fetchUpdateProductAttribute(data)).unwrap();
    ...
    const fetchData = async () => {
      try {
        ...
        const { data } = await dispatch(
          fetchGetUpdateProductAttributeByID(productAttributeId),
        ).unwrap();
        await dispatch(
          fetchGetAllColor({
            pageSize: 100,
